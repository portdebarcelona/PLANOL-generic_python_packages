...
from apb_extra_osgeo_utils import feats_layer_gdal, geoms_layer_gdal, PREFFIX_GEOMS_LAYERS_GDAL, pg_string_connection, \
    datasource_gdal_vector_file, copy_layer_gdal_to_ds_gdal, get_ds_postgis, add_layer_gdal_to_ds_gdal, \
    layer_gdal_from_file, fix_affix_geom_name_layer_gdal
...
class TestOsgeoUtils(unittest.TestCase):
    ds_gpkg, overwrite = datasource_gdal_vector_file(
    ...
    def test_layer_geojson(self):
        ds, ovrwrt = datasource_gdal_vector_file(
    ...
    def test_copy_layer(self):
        ds_dest, ovrwrt = datasource_gdal_vector_file(
        ...
        lyr_dest = copy_layer_gdal_to_ds_gdal(lyr_orig, ds_dest)
    ...
    def test_postgis_conn(self):
        ds = get_ds_postgis(dbname='GIS', host=self.host_pg, port=self.port_pg,
                                                  user=USER_DB_POSTGRES, password=PASSWORD_DB_POSTGRES)
        self.assertIsNotNone(ds)

        os.environ['PGSERVICEFILE'] = os.path.join(os.path.dirname(__file__), 'pg_service.conf')
        ds = get_ds_postgis(
            service='postgis-localhost-docker',
            password=PASSWORD_DB_POSTGRES,
        )
        self.assertIsNotNone(ds)

        pg_conn = pg_string_connection(
            service='postgis-localhost-docker',
            password=PASSWORD_DB_POSTGRES,)
        ds = get_ds_postgis(pg_conn_str=pg_conn)
        self.assertIsNotNone(ds)
    ...
    def test_add_layer_mono_geom_to_postgis(self):
        ...
        ds = get_ds_postgis(dbname='GIS', host=self.host_pg, port=self.port_pg,
                                                  user=USER_DB_POSTGRES, password=PASSWORD_DB_POSTGRES)
        lyrs_dest = add_layer_gdal_to_ds_gdal(
    ...
    def test_add_layer_multi_geom_to_postgis(self):
        ds_csv, ovrwrt = datasource_gdal_vector_file(
            'CSV', 'edificacio.zip', path_data, create=False, from_zip=True)
    ...
    def test_create_mono_geom_from_csv(self):
        layer_gdal, nom_layer_gdal, ds_gdal = layer_gdal_from_file(
            os.path.join(path_data, 'edificacio.zip'), 'CSV', default_order_long_lat=False)
        for nom_geom in geoms_layer_gdal(layer_gdal):
            fix_nom_geom = fix_affix_geom_name_layer_gdal(nom_geom, layer_gdal)
            ds_gdal_geojson, overwrited = datasource_gdal_vector_file(
                'GEOJSON',
                "{}-{}".format(nom_layer_gdal, fix_nom_geom.lower()),
                path_data,
                create=True)
            add_layer_gdal_to_ds_gdal(ds_gdal_geojson, layer_gdal, lite=True, srs_epsg_code=4326,
                                                  nom_geom=nom_geom)

    def test_create_csv_from_csv(self):
        layer_gdal, nom_layer_gdal, ds_gdal = layer_gdal_from_file(
            os.path.join(path_data, 'edificacio.zip'), 'CSV', default_order_long_lat=True)
        ds_gdal_csv, overwrited = datasource_gdal_vector_file(
            'CSV',
            "{}_{}".format(nom_layer_gdal, 25831),
            path_data,
            create=True)
        add_layer_gdal_to_ds_gdal(ds_gdal_csv, layer_gdal, lite=True, srs_epsg_code=25831, multi_geom=True)
