      ...
      steps {
        script {
          ...
          if (env.REPO_BRANCH == 'training') {
            ...
            docker.withRegistry("", "${DOCKER_REGISTRY_CREDENTIALS}") {
            image = docker.build("${DOCKER_ALL_PACKAGES_URL}", "--build-arg ARG_DOCKER_TAG=${tag} --no-cache -f Dockerfile.dev .")
            image.push(tag)
          }
        }
      }
    ...
    stage('Docker build & push (TAG RELEASE)') {
      ...
      steps {
        script {
            ...
            image.push("latest")
            image = docker.build("${DOCKER_ALL_PACKAGES_URL}", "--build-arg ARG_DOCKER_TAG=latest --no-cache -f Dockerfile.dev .")
            image.push("${TAG_RELEASE}")
            image.push("latest")
          }
        }
